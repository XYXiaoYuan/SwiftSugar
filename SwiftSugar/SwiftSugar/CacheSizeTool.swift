...
class CacheSizeTool: NSObject {
    
    static func removeCacheFilesByDirectoryPath(directoryPath: String) {
        // 获取文件管理者
    ...
    static func getCacheFileSize(directoryPath: String, complection: @escaping (cacheSize<String>) -> Void) {
        // 获取文件管理者
        let manger = FileManager.default
        var isDirectory: ObjCBool = false;
        let isExist = manger.fileExists(atPath: directoryPath, isDirectory: &isDirectory)
